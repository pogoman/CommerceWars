# Builds jars/CommWars.jar from src/. Requires a JDK (javac + jar) on PATH,
...
& $jar cf (Join-Path $PSScriptRoot "jars\CommWars.jar") -C $out .
if ($LASTEXITCODE -ne 0) { throw "jar failed" }

Write-Host "Built jars\CommWars.jar"
